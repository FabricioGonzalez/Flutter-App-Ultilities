// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'dialog_todo_controller.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic

mixin _$DialogTodoController on ControllerBase, Store {
  final _$taskAtom = Atom(name: 'ControllerBase.task');

  @override
  String get task {
    _$taskAtom.reportRead();
    return super.task;
  }

  @override
  set task(String value) {
    _$taskAtom.reportWrite(value, super.task, () {
      super.task = value;
    });
  }

  final _$limitDateAtom = Atom(name: 'ControllerBase.limitDate');

  @override
  String get limitDate {
    _$limitDateAtom.reportRead();
    return super.limitDate;
  }

  @override
  set limitDate(String value) {
    _$limitDateAtom.reportWrite(value, super.limitDate, () {
      super.limitDate = value;
    });
  }

  final _$ControllerBaseActionController =
      ActionController(name: 'ControllerBase');

  @override
  void setTask(String task) {
    final _$actionInfo = _$ControllerBaseActionController.startAction(
        name: 'ControllerBase.setTask');
    try {
      return super.setTask(task);
    } finally {
      _$ControllerBaseActionController.endAction(_$actionInfo);
    }
  }

  @override
  void setLimitDate(String limitDate) {
    final _$actionInfo = _$ControllerBaseActionController.startAction(
        name: 'ControllerBase.setLimitDate');
    try {
      return super.setLimitDate(limitDate);
    } finally {
      _$ControllerBaseActionController.endAction(_$actionInfo);
    }
  }

  @override
  String toString() {
    return '''
task: ${task},
limitDate: ${limitDate}
    ''';
  }
}
